...
include '../includes/Application.php';
	$db = new cDB();
	$hwid    = $_GET['hwid'];
	$typeid  = $_GET['printertypeid'];
	$makeid  = $_GET['makeid'];
	$modelid = $_GET['modelid'];
	$db->Query("
	SELECT he.empid empid, he.empname empname, he.unit unit, he.department department, he.designation designation,hs.activestatus
	FROM `hz_registration` hs
	LEFT OUTER JOIN hz_employees he ON ( hs.empid = he.empid )
	WHERE hs.hardware = $hwid AND hs.printertype = $typeid AND hs.make = $makeid AND hs.model = $modelid AND hs.activestatus IN('A','T')
	");
?>
...
<div id="container-2" class="box1">
	<table cellpadding="0" cellspacing="0" border="0" class="display" id="ListAvailStock">
		...
		if ($db->RowCount){
			while ($db->ReadRow()) {
				if ($db->RowData['activestatus'] == 'A') {
					$status = 'ACTIVE';
				} else if ($db->RowData['activestatus'] == 'T') {
					$status = 'TRANSFERRED';
				}
				echo "<tr>";
