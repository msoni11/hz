...
if (!isset($_SESSION['username']) || $_SESSION['isadmin'] != 1) {
	...
	if (isset($_POST['dosearch']) && $_POST['dosearch'] != 'dosearch') {
		...
		$ORDERBY = '';
		$GROUPBY = '';
		if ($_POST['txtdepartment'] == '-1' && $_POST['txtunit'] == '-1' && $_POST['txthardware'] == '-1') {
			$deptflag = true;
			$unitflag = true;
			$hwflag = true;
		...
		if (((isset($_POST['txtstartday']) && $_POST['txtstartday'] != '-1') && (isset($_POST['txtstartmonth']) && $_POST['txtstartmonth'] != '-1') && (isset($_POST['txtstartyear']) && $_POST['txtstartyear'] != '-1'))
			...
			if (($hwflag) || ($unitflag) || ($deptflag) || ($deptflag && $unitflag)) {
...
<div id="container-2" class="box1">
	...
	<div id="stockbydeptlist" style="padding:50px 0 0 0;display:block;">
		<form name="searchdept" action="stockbydepartment.php" method="post">
		<table border="0" id="test" width="200%">
		<tr>
			<th rowspan="2" width="10%">Filter By Department:</th>
			<th rowspan="2" width="15%">Filter By Unit:</th>
			<th rowspan="2" width="15%">Filter By hardware:</th>
			<th colspan="2" width="60%" >Months between:</th>
		</tr>
		<tr>
			<th>Start:</th>
			<th>End:</th>
		...
		<tr>
			<td colspan="4"><input type="submit" name="dosearch" id="dosearch" value="Search" >&nbsp;<input type="button" name="remfilter" id="remfilter" value="Remove Filter" onclick="window.location='stockbydepartment.php';" ></td>
		</tr>
		</table>
		</form>
