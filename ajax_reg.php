...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
	{
		...
		{
			if ($aColumns[$i] == 'empid') {
				$sWhere .= "he.empid = '".mysql_real_escape_string( $_GET['sSearch'] )."' OR ";
	 ...
	 */
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS he.empid as empid, empname, unit, department, designation, hardware, cartage, printertype, make, model, cpuno, monitortype, monitorno, sysconfig, cartage, assetcode, ipaddr, officever, licensesoft, internet, internettype, warnorvendor, date, otheritasset, status, hr.id as id
		FROM   $sTable ON (hr.empid = he.empid)
		$sWhere
		$sOrder
		$sLimit
		";
